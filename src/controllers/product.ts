  ...
  try {
    ...
    if (!name || !price || !category || !stock) {
      ...
      rm(photo.path, (err) => {
        ...
        else logger.info(`File deleted: ${photo.path}`);
      });
  ...
  try {
    ...
    if (photo) {
      ...
      rm(product.photo!, (err) => {
        if (err) logger.error(`Failed to delete old photo: ${product.photo}`, { stack: err.stack });
        else logger.info(`Old photo deleted: ${product.photo}`);
      });
      // Add new photo
...
export const deleteProduct = async (
  ...
  try {
    ...
    rm(product.photo!, (err) => {
      if (err) logger.error(`Failed to delete product photo: ${product.photo}`, { stack: err.stack });
      else logger.info(`Product photo deleted: ${product.photo}`);
    });
